...
static int ssd1306_write(const struct device *dev,
                         const uint16_t x,
                         const uint16_t y,
                         ...
                         const void *buf)
{
  ...
  buf_len = desc->width * ((desc->height + 7) / 8);
  ...
  for (uint8_t n = 0; n < ((desc->height + 7) / 8); n++) {
    cmd_buf[sizeof(cmd_buf) - 1] =
      SSD1306_SET_PAGE_START_ADDRESS + n;
...
static int ssd1306_set_contrast(const struct device *dev,
