...
#define UI_PRINTF(...)   printk(__VA_ARGS__)
...
#define UI_PRINTBOX(...) printk(__VA_ARGS__)
  ...
  for (row = row_begin; row < row_end; row++) {
    ...
    switch (row) {
      ...
      case ROW_RSSI:
        ...
        break;
      case ROW_INTERVAL:
        UI_PRINTF(THROUGHPUT_UI_INTERVAL_FORMAT,
                  (int)(ui_state.interval * 1.25));
        UI_PRINTBOX("  ");
        break;
      case ROW_PDU_SIZE:
